 ...
 *          snapshot        context
 *          --------------  -----------
 * object   get-snapshot    n/a
 * global   set-snapshot    set-context
 * with []  with-snapshot   with-context
 * 
 * context primitives would require dirty flag
 * performance could be affected if graph is dirtied during bulk operations
 * 
 */
